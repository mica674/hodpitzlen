...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { //Si les données sont bien envoyées en POST
    ...
    if (!Appointment::isIdExist($idAppointment)) {
        $error['appointment'] = 'Le rendez-vous n\'existe pas';
    }
...
try {
    if (!isset($patient)) {
        $idAppointment = intval(filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT));
        if (!Appointment::isIdExist($idAppointment)) {
           header('Location: /AppointmentsList?exist=false');die;
        }else {   
            $appointment = Appointment::get($idAppointment);
            $idPatient = $appointment->idPatients;
            $dateAppointment = $appointment->dateHour;
    ...
    if ($deleteOK ?? false) {
        $appointmentDeleted = Appointment::delete($idAppointment);
        header('Location: /AppointmentsList?deleted=true');die;
    }
} catch (\Throwable $th) {
...
// Appel de la view
include(__DIR__ . '/../views/appointments/deleteAppointment.php');
